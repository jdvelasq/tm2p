...
Smoke tests:
    >>> from tm2p.analyze.metrics.terms_by_year import GanttPlot
    >>> plotter = (
    ...     GanttPlot()
    ...     #
    ...     # FIELD:
    ...
    ... )
    >>> plot = plotter.run()
    >>> plot.write_html("docsrc/_generated/px.database.metrics.terms_by_year.gantt_plot.html")
...
class GanttChart(
    ...
    def internal__create_gantt_diagram(self):
        ...
        fig.update_layout(
            ...
            showlegend=False,
            xaxis_title=None,
            yaxis_title=self.params.source_field.value.replace("_", " ").upper(),
        )
        fig.update_traces(
            marker={
