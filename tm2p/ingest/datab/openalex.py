...
class OpenAlex(BaseIngest):
    ...
    def ingestion_pipeline(self) -> tuple[tuple[str, list[Step]], ...]:
        ...
        from ._intern.phases.p03_pars import build_openalex_parsing_steps
        from ._intern.phases.p04_struct_stand import build_openalex_struct_stand_steps
        from ._intern.phases.p05_field_normal import build_openalex_field_normal_steps

        return (
            (self._COMPRESS, build_openalex_compress_steps(self.params)),
            (self._PARSING, build_openalex_parsing_steps(self.params)),
            (self._STRUCT_STAND, build_openalex_struct_stand_steps(self.params)),
            (self._FIELD_NORMAL, build_openalex_field_normal_steps(self.params)),
            #
            #
