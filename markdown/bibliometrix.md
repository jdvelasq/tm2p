...
tm2+ uses some basic algorithms to found matches between the references in free text in the records of the analyzed dataset, with the a dataset of references. tm2+ search for simultaneous matches in year, first author surname and part of the title. With this information, a thesaurus of references is generated. The user can modify and apply this thesaurus to the dataset to modify the references. The thesaurus implements fuzzy-cutoff matching and other features, which are implemented based on VantagePoint.
